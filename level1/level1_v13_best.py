"""
Level 1 v13 — Core Demand Prediction (E-Class) — LightGBM Supervised
Strategy: EXPAND pool to reach 49k predictions (v9 capped at 44k due to pool size)
  - v9 base (proven best at €1,479k)
  - Loosened EV floors to include more marginal-but-profitable items
  - Expanded cold candidates (lower min_spend, more per buyer)
  - Added eclass hierarchy sharing
  - Target 49k (matching #1's 48,844)
"""
import pandas as pd
import numpy as np
...
OUTPUT_PATH = 'submission_level1_v13.csv'
...
df['eclass'] = pd.to_numeric(df['eclass'], errors='coerce')
df = df.dropna(subset=['eclass'])
df['eclass'] = df['eclass'].astype(int).astype(str)
